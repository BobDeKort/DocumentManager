...
class NetworkLayer {
    ...
    func downloadZipForCollection(collection: Collection, completionHandler: @escaping (URL) -> ()) {
        let destination: DownloadRequest.DownloadFileDestination = { _, _ in
            let temURL = try! FileManager.default.url(for: FileManager.SearchPathDirectory.itemReplacementDirectory, in: FileManager.SearchPathDomainMask.userDomainMask, appropriateFor: nil, create: false)
            
            return (temURL, [.removePreviousFile])
        }
